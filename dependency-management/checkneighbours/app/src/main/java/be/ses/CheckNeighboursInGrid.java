...
public class CheckNeighboursInGrid {
    ...
    public static Iterable<Integer> getSameNeighboursIds(Iterable<Integer> grid, int width, int height, int indexToCheck) {
        ...
        if (col > 0 && gridList.get(indexToCheck - 1) == valueToCheck) {
            result.add(indexToCheck - 1);
        }
        if (col < width - 1 && gridList.get(indexToCheck + 1) == valueToCheck) {
            result.add(indexToCheck + 1);
        }
        if (row > 0 && gridList.get(indexToCheck - width) == valueToCheck) {
            result.add(indexToCheck - width);
        }
        if (row < height - 1 && gridList.get(indexToCheck + width) == valueToCheck) {
            result.add(indexToCheck + width);
        }
